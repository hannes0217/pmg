...
class PMGApiClient:
    ...
    async def async_post(self, path: str, data: dict[str, Any] | None = None) -> Any:
        if self._auth is None:
            await self.async_login()
        ...
        try:
            async with self._session.post(
                ...
                if resp.status == 401:
                    ...
                    async with self._session.post(
                        ...
                        try:
                            ...
                            text = await retry_resp.text()
                            raise PMGApiError(
                                f"POST {path} failed: {retry_resp.status} {text}"
                            ) from None
                        if retry_resp.status != 200:
                            raise PMGApiError(
                                f"POST {path} failed: {retry_resp.status} {payload}"
                            )
                        return payload.get("data")
                ...
                try:
                    ...
                    text = await resp.text()
                    raise PMGApiError(
                        f"POST {path} failed: {resp.status} {text}"
                    ) from None
                if resp.status != 200:
                    raise PMGApiError(f"POST {path} failed: {resp.status} {payload}")
        except (ClientError, ContentTypeError, asyncio.TimeoutError) as err:
            raise PMGApiError(f"POST {path} failed: {err}") from err
